...
# We leave zephyr base but we have to change some paths below in this script
ZEPHYR_BASE = os.environ.get("ZEPHYR_BASE")

# This is our project base. We have to make appropriate adjustments as well
PROJECT_BASE = os.path.realpath(os.path.dirname(os.path.realpath(__file__)) + "/../../")
...
def get_shas(refspec):
    ...
    """
    return git(
        "rev-list", "--max-count={}".format(-1 if "." in refspec else 1), refspec
    ).split()
...
class CheckPatch(ComplianceTest):
    ...
    def run(self):
        checkpatch = os.path.join(PROJECT_BASE, "scripts", "checkpatch.pl")
        if not os.path.exists(checkpatch):
            self.skip(checkpatch + " not found")

        # git diff's output doesn't depend on the current (sub)directory
        diff = subprocess.Popen(("git", "diff", COMMIT_RANGE), stdout=subprocess.PIPE)
        try:
...
class KconfigCheck(ComplianceTest):
    ...
    def get_modules(self, modules_file):
        ...
        zephyr_module_path = os.path.join(ZEPHYR_BASE, "scripts", "zephyr_module.py")
        cmd = [sys.executable, zephyr_module_path, "--kconfig-out", modules_file]
        try:
        ...
        modules = [
            ...
            if os.path.exists(os.path.join(modules_dir, name, "Kconfig"))
        ]
...
""".format(
                    max_top_items, n_top_items, __file__
                )
            )
...
"""
                + "\n".join(
                    f"{node.item.name:35} {node.filename}:{node.linenr}"
        ...
        for grep_line in grep_stdout.splitlines():
            ...
            for sym_name in re.findall(regex, line):
                ...
                if (
                    ...
                    and sym_name not in UNDEF_KCONFIG_WHITELIST
                ):
...
{}""".format(
                os.path.basename(__file__), undef_desc
            )
        )
...
class Codeowners(ComplianceTest):
    ...
    def ls_owned_files(self, codeowners):
        ...
        with open(codeowners, "r") as codeo:
            for lineno, line in enumerate(codeo, start=1):
                ...
                if not files:
                    self.add_failure(
                        "Path '{}' not found in the tree but is listed in "
    ...
    def git_pattern_to_glob(self, git_pattern):
        ...
        if git_pattern.endswith("/"):
            ...
            self.add_failure(
                "Expected '/' after directory '{}' " "in CODEOWNERS".format(ret)
            )
    ...
    def run(self):
        ...
        name_changes = git("diff", "--name-only", "--diff-filter=ARCD", COMMIT_RANGE)

        owners_changes = git("diff", "--name-only", COMMIT_RANGE, "--", codeowners)
        ...
        # Addition instead.
        new_files = git(
            "diff", "--name-only", "--diff-filter=ARC", COMMIT_RANGE
        ).splitlines()
        ...
        if new_not_owned:
            self.add_failure(
                "New files added that are not covered in "
                "CODEOWNERS:\n\n"
                + "\n".join(new_not_owned)
...
concatenated together, so no document separators are needed."""
    ...
    def check_source_file(self, fname):
        ...
        if not contents.endswith("\n"):
            self.add_failure(
                "Missing newline at end of '{}'. Check your text "
                "editor settings.".format(fname)
            )

        if contents.startswith("\n"):
            self.add_failure("Please remove blank lines at start of '{}'".format(fname))

        if contents.endswith("\n\n"):
            self.add_failure("Please remove blank lines at end of '{}'".format(fname))
...
class PyLint(ComplianceTest):
    ...
    def run(self):
        ...
        pylintrc = ZEPHYR_BASE + "/scripts/ci/pylintrc"
        ...
        try:
            ...
            process = subprocess.Popen(
                pylintcmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=GIT_TOP
            )
        except OSError as e:
...
class Identity(ComplianceTest):
    ...
    def run(self):
        for shaidx in get_shas(COMMIT_RANGE):
            ...
            error1 = (
                "%s: author email (%s) needs to match one of the signed-off-by entries."
                % (sha, author)
            )
            error2 = (
                "%s: author email (%s) does not follow the syntax: First Last <email>."
                % (sha, author)
            )
            error3 = (
                "%s: author email (%s) must be a real email and cannot end in @users.noreply.github.com"
                % (sha, author)
            )
            failure = None
...
def init_logs(cli_arg):
    ...
    logging.info(
        "Log init completed, level=%s", logging.getLevelName(logger.getEffectiveLevel())
    )
...
def parse_args():
    ...
    parser.add_argument("-r", "--repo", default=None, help="GitHub repository")
    parser.add_argument(
        "-p", "--pull-request", default=0, type=int, help="Pull request number"
    )
    ...
    parser.add_argument(
        "-l", "--list", action="store_true", help="List all checks and exit"
    )
...
def _main(args):
    ...
    if args.previous_run:
        if not os.path.exists(args.previous_run):
            ...
            print("error: '{}' not found".format(args.previous_run), file=sys.stderr)
            return 1
    ...
    failed_cases = []
    name2doc = {
        testcase.name: testcase.doc for testcase in ComplianceTest.__subclasses__()
    }
