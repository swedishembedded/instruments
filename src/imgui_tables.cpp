...
void ImGui::TableUpdateLayout(ImGuiTable *table)
{
	...
	table->MinColumnWidth = ImMax(
		1.0f, g.Style.FramePadding.x * 1.0f); // g.Style.ColumnsMinSpacing; // FIXME-TABLE
	...
	float offset_x =
		((table->FreezeColumnsCount > 0) ? table->OuterRect.Min.x : work_rect.Min.x) +
	...
	for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
		...
		column->WidthGiven = ImMax(column->WidthGiven,
	...
	const float unused_x1 = ImMax(table->WorkRect.Min.x,
...
void ImGui::EndTable()
{
	...
	if (temp_data->UserOuterSize.y <= 0.0f) {
		...
		outer_window->DC.CursorMaxPos.y = ImMax(
			backup_outer_max_pos.y, ImMin(table->OuterRect.Max.y, inner_content_max_y));
	} else {
...
void ImGui::TableMergeDrawChannels(ImGuiTable *table)
{
	...
	if (merge_group_mask != 0) {
		...
		int remaining_count = splitter->_Count - (has_freeze_v ? LEADING_DRAW_CHANNELS + 1 :
			...
			if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount) {
				...
				for (int n = 0; n < IM_ARRAYSIZE(remaining_mask.Storage); n++)
					remaining_mask.Storage[n] &=
						~merge_group->ChannelsMask.Storage[n];
...
void ImGui::TableDrawBorders(ImGuiTable *table)
{
	...
	if (table->Flags & ImGuiTableFlags_BordersOuter) {
		...
		if ((table->Flags & ImGuiTableFlags_BordersOuter) == ImGuiTableFlags_BordersOuter) {
			inner_drawlist->AddRect(outer_border.Min, outer_border.Max, outer_col, 0.0f,
						0, border_size);
...
void ImGui::TableSortSpecsBuild(ImGuiTable *table)
{
	...
	if (dirty) {
		...
		table->SortSpecsMulti.resize(table->SortSpecsCount <= 1 ? 0 :
...
void ImGui::TableHeader(const char *label)
{
	...
	if (held || hovered || selected) {
		...
		if ((table->RowFlags & ImGuiTableRowFlags_Headers) == 0)
			TableSetBgColor(ImGuiTableBgTarget_CellBg,
					GetColorU32(ImGuiCol_TableHeaderBg), table->CurrentColumn);
	}
	...
	RenderTextEllipsis(window->DrawList, label_pos,
			   ImVec2(ellipsis_max,
				  label_pos.y + label_height + g.Style.FramePadding.y),
...
void ImGui::TableDrawContextMenu(ImGuiTable *table)
{
	...
	if (table->Flags & ImGuiTableFlags_Resizable) {
		if (column != NULL) {
			const bool can_resize = !(column->Flags & ImGuiTableColumnFlags_NoResize) &&
						column->IsEnabled;
...
void ImGui::DebugNodeTable(ImGuiTable *table)
{
	...
	BulletText("OuterRect: Pos: (%.1f,%.1f) Size: (%.1f,%.1f) Sizing: '%s'",
		   table->OuterRect.Min.x, table->OuterRect.Min.y, table->OuterRect.GetWidth(),
	...
	BulletText("ResizedColumn: %d, ReorderColumn: %d, HeldHeaderColumn: %d",
		   table->ResizedColumn, table->ReorderColumn, table->HeldHeaderColumn);
...
static float GetDraggedColumnOffset(ImGuiOldColumns *columns, int column_index)
{
	...
	float x = g.IO.MousePos.x - g.ActiveIdClickOffset.x + COLUMNS_HIT_RECT_HALF_WIDTH -
		  window->Pos.x;
...
void ImGui::SetColumnOffset(int column_index, float offset)
{
	...
	const float width =
		preserve_width ? GetColumnWidthEx(columns, column_index, columns->IsBeingResized) :
				 0.0f;

	if (!(columns->Flags & ImGuiOldColumnFlags_NoForceWithinWindow))
		offset = ImMin(offset, columns->OffMaxX - g.Style.ColumnsMinSpacing *
...
void ImGui::EndColumns()
{
	...
	if (!(flags & ImGuiOldColumnFlags_NoBorder) && !window->SkipItems) {
		...
		for (int n = 1; n < columns->Count; n++) {
			...
			if (IsClippedEx(
				    column_hit_rect,
				    column_id)) // FIXME: Can be removed or replaced with a lower-level test
				continue;
