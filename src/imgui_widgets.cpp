...
void ImGui::LabelTextV(const char *label, const char *fmt, va_list args)
{
	...
	const ImRect total_bb(
		pos,
		pos + ImVec2(w + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x :
							0.0f),
...
void ImGui::BulletTextV(const char *fmt, va_list args)
{
	...
	RenderBullet(window->DrawList,
		     ...
		     text_col);
	RenderText(bb.Min + ImVec2(g.FontSize + style.FramePadding.x * 2, 0.0f), text_begin,
		   text_end, false);
}
			   ...
			   ImGuiButtonFlags flags)
{
	...
	if (g.NavActivateDownId == id) {
		...
		if (!nav_activated_by_inputs && (flags & ImGuiButtonFlags_Repeat)) {
			...
			nav_activated_by_inputs = CalcTypematicRepeatAmount(t1 - g.IO.DeltaTime, t1,
									    g.IO.KeyRepeatDelay,
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2 &pos)
{
	...
	if (hovered)
		window->DrawList->AddCircleFilled(center, ImMax(2.0f, g.FontSize * 0.5f + 1.0f),
						  col, 12);
			...
			const ImVec4 &tint_col)
{
	...
	return ImageButtonEx(window->GetID(str_id), user_texture_id, size, uv0, uv1, bg_col,
			     tint_col);
}
...
bool ImGui::Checkbox(const char *label, bool *v)
{
	...
	const ImVec2 pos = window->DC.CursorPos;
	const ImRect total_bb(
		pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ?
...
void ImGui::ProgressBar(float fraction, const ImVec2 &size_arg, const char *overlay)
{
	...
	ImVec2 pos = window->DC.CursorPos;
	ImVec2 size =
		CalcItemSize(size_arg, CalcItemWidth(), g.FontSize + style.FramePadding.y * 2.0f);
...
bool ImGui::BeginCombo(const char *label, const char *preview_value, ImGuiComboFlags flags)
{
	...
	if (!(flags & ImGuiComboFlags_NoPreview))
		window->DrawList->AddRectFilled(bb.Min, ImVec2(value_x2, bb.Max.y), frame_col,
						style.FrameRounding,
...
bool ImGui::DragFloat2(const char *label, float v[2], float v_speed, float v_min, float v_max,
		       const char *format, ImGuiSliderFlags flags)
{
	return DragScalarN(label, ImGuiDataType_Float, v, 2, v_speed, &v_min, &v_max, format,
			   flags);
}
...
bool ImGui::DragFloat4(const char *label, float v[4], float v_speed, float v_min, float v_max,
		       const char *format, ImGuiSliderFlags flags)
{
	return DragScalarN(label, ImGuiDataType_Float, v, 4, v_speed, &v_min, &v_max, format,
			   flags);
}
			    ...
			    const char *format_max, ImGuiSliderFlags flags)
{
	...
	bool value_changed = DragScalar("##min", ImGuiDataType_Float, v_current_min, v_speed,
		       ...
		       const void *p_min, const void *p_max, const char *format, float power)
{
	...
	if (power != 1.0f) {
		...
		IM_ASSERT(
			p_min != NULL &&
			p_max !=
				NULL); // When using a power curve the drag needs to have known bounds
		...
		if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
			if (!g.IO.MouseDown[0]) {
				...
				if (slider_usable_sz > 0.0f)
					clicked_t = ImSaturate((mouse_abs_pos -
								g.SliderGrabClickOffset -
			   ...
			   ImGuiSliderFlags flags, ImRect *out_grab_bb)
{
	...
	case ImGuiDataType_S16: {
		...
		bool r = SliderBehaviorT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32,
							      *(const ImS16 *)p_min,
							      *(const ImS16 *)p_max, format, flags,
							      out_grab_bb);
		if (r)
			*(ImS16 *)p_v = (ImS16)v32;
		return r;
	...
	case ImGuiDataType_U16: {
		...
		bool r = SliderBehaviorT<ImU32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32,
							      *(const ImU16 *)p_min,
							      *(const ImU16 *)p_max, format, flags,
							      out_grab_bb);
		if (r)
			*(ImU16 *)p_v = (ImU16)v32;
		return r;
	...
	case ImGuiDataType_Float:
		...
		return SliderBehaviorT<float, float, float>(bb, id, data_type, (float *)p_v,
							    *(const float *)p_min,
							    *(const float *)p_max, format, flags,
							    out_grab_bb);
	case ImGuiDataType_Double:
			 ...
			 ImGuiSliderFlags flags)
{
	...
	// Slider behavior
	ImRect grab_bb;
	const bool value_changed = SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max,
...
const char *ImParseFormatSanitizeForScanning(const char *fmt_in, char *fmt_out, size_t fmt_out_size)
{
	...
	while (fmt_in < fmt_end) {
		...
		if (!has_type && ((c >= '0' && c <= '9') || c == '.'))
			continue;
		has_type |=
			((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')); // Stop skipping digits
		if (c != '\'' && c != '$' &&
		    c != '_') // Custom flags provided by stb_sprintf.h. POSIX 2008 also supports '.
			*(fmt_out++) = c;
	}
		      ...
		      ImGuiInputTextCallback callback, void *user_data)
{
	...
	return InputTextEx(label, NULL, buf, (int)buf_size, ImVec2(0, 0), flags, callback,
			   user_data);
}
			      ...
			      void *user_data)
{
	...
	return InputTextEx(label, hint, buf, (int)buf_size, ImVec2(0, 0), flags, callback,
			   user_data);
}
			...
			ImGuiInputTextCallback callback, void *callback_user_data)
{
	...
	const bool init_make_active = (user_clicked || user_scroll_finish ||
				       input_requested_by_nav || input_requested_by_tabbing);
	...
	if (is_readonly && state != NULL && (render_cursor || render_selection)) {
		...
		state->CurLenW = ImTextStrFromUtf8(state->TextW.Data, state->TextW.Size, buf, NULL,
						   &buf_end);
	...
	if (g.ActiveId == id) {
		...
		if ((flags & ImGuiInputTextFlags_AllowTabInput) && IsKeyPressed(ImGuiKey_Tab) &&
	...
	if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
		...
		const bool is_cancel =
			IsKeyPressed(ImGuiKey_Escape, false) ||
		...
		if (IsKeyPressed(ImGuiKey_LeftArrow)) {
			...
			if (io.KeyCtrl)
				SetScrollY(draw_window, ImMin(draw_window->Scroll.y + g.FontSize,
							      GetScrollMaxY()));
			else
			...
			if ((flags & (ImGuiInputTextFlags_CallbackCompletion |
				...
				if (event_flag) {
					...
					if (buf_dirty) {
						...
						state->CurLenW =
							ImTextStrFromUtf8(state->TextW.Data,
									  state->TextW.Size,
	...
	if (render_cursor || render_selection) {
...

		{
			...
			if (render_selection) {
				searches_input_ptr[1] = text_begin + ImMin(state->Stb.select_start,
									   state->Stb.select_end);
			...
			if (searches_result_line_no[1] >= 0) {
				select_start_offset.x =
					InputTextCalcTextSizeW(ImStrbolW(searches_input_ptr[1],
									 text_begin),
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState *state)
{
	...
	Text("CurLenW: %d, CurLenA: %d, Cursor: %d, Selection: %d..%d", state->CurLenA,
	     state->CurLenW, stb_state->cursor, stb_state->select_start, stb_state->select_end);
...
bool ImGui::ColorEdit4(const char *label, float col[4], ImGuiColorEditFlags flags)
{
	...
	if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 &&
		...
		const float w_item_one = ImMax(
			1.0f, IM_FLOOR((w_inputs - (style.ItemInnerSpacing.x) * (components - 1)) /
				       (float)components));
		const float w_item_last =
			ImMax(1.0f, IM_FLOOR(w_inputs - (w_item_one + style.ItemInnerSpacing.x) *
								(components - 1)));
		...
		if (IsItemActive()) {
			...
			if (ImTriangleContainsPoint(triangle_pa, triangle_pb, triangle_pc,
				...
				if (!ImTriangleContainsPoint(triangle_pa, triangle_pb, triangle_pc,
					...
					current_off_unrotated =
						ImTriangleClosestPoint(triangle_pa, triangle_pb,
								       triangle_pc,
								       current_off_unrotated);
				float uu, vv, ww;
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel) {
		...
		for (int n = 0; n < 6; n++) {
			...
			ShadeVertsLinearColorGradientKeepAlpha(draw_list, vert_start_idx,
							       vert_end_idx, gradient_p0,
							       gradient_p1, col_hues[n],
							       col_hues[n + 1]);
		}
		...
		draw_list->AddRectFilledMultiColor(
			picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), col_white,
			hue_color32, hue_color32, col_white);
		draw_list->AddRectFilledMultiColor(
			picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), 0, 0,
			col_black, col_black);
		RenderFrameBorder(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size),
				  0.0f);
		sv_cursor_pos.x =
		...
		RenderArrowsForVerticalBar(draw_list, ImVec2(bar0_pos_x - 1, bar0_line_y),
					   ImVec2(bars_triangles_half_sz + 1,
	...
	if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f) {
		...
		if (col_source.w < 1.0f)
			RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min,
							     bb_inner.Max, GetColorU32(col_source),
...
void ImGui::SetColorEditOptions(ImGuiColorEditFlags flags)
{
	...
	IM_ASSERT(ImIsPowerOfTwo(
		flags & ImGuiColorEditFlags_DisplayMask_)); // Check only 1 option is selected
	IM_ASSERT(ImIsPowerOfTwo(
		flags & ImGuiColorEditFlags_DataTypeMask_)); // Check only 1 option is selected
	IM_ASSERT(ImIsPowerOfTwo(
		flags & ImGuiColorEditFlags_PickerMask_)); // Check only 1 option is selected
	IM_ASSERT(ImIsPowerOfTwo(
		flags & ImGuiColorEditFlags_InputMask_)); // Check only 1 option is selected
...
void ImGui::ColorEditOptionsPopup(const float *col, ImGuiColorEditFlags flags)
{
	...
	if (BeginPopup("Copy")) {
		int cr = IM_F32_TO_INT8_SAT(col[0]), cg = IM_F32_TO_INT8_SAT(col[1]),
		    ...
		    ca = (flags & ImGuiColorEditFlags_NoAlpha) ? 255 : IM_F32_TO_INT8_SAT(col[3]);
		char buf[64];
		ImFormatString(buf, IM_ARRAYSIZE(buf), "(%.3ff, %.3ff, %.3ff, %.3ff)", col[0],
			       col[1], col[2],
			       (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : col[3]);
		if (Selectable(buf))
...
bool ImGui::TreeNodeUpdateNextOpen(ImGuiID id, ImGuiTreeNodeFlags flags)
{
	...
	if (g.NextItemData.Flags & ImGuiNextItemDataFlags_HasOpen) {
		...
		is_open =
			storage->GetInt(id, (flags & ImGuiTreeNodeFlags_DefaultOpen) ? 1 : 0) != 0;
	}
...
bool ImGui::CollapsingHeader(const char *label, bool *p_visible, ImGuiTreeNodeFlags flags)
{
	...
	if (p_visible != NULL) {
		...
		float button_x = ImMax(g.LastItemData.Rect.Min.x,
				       g.LastItemData.Rect.Max.x - g.Style.FramePadding.x * 2.0f -
					       button_size);
		       ...
		       const ImVec2 &size_arg)
{
	...
	const bool item_add = ItemAdd(
		bb, id, NULL, disabled_item ? ImGuiItemFlags_Disabled : ImGuiItemFlags_None);
	if (span_all_columns) {
	...
	RenderTextClipped(text_min, text_max, label, NULL, &label_size, style.SelectableTextAlign,
			  &bb);
				 ...
				 float axis_size, ImGuiWindowFlags window_flags)
{
	...
	if (bar_window == NULL || bar_window->BeginCount == 0) {
		...
		ImGuiAxis axis =
			(dir == ImGuiDir_Up || dir == ImGuiDir_Down) ? ImGuiAxis_Y : ImGuiAxis_X;
...
bool ImGui::BeginTabBarEx(ImGuiTabBar *tab_bar, const ImRect &tab_bar_bb, ImGuiTabBarFlags flags)
{
	...
	const float y = tab_bar->BarRect.Max.y - 1.0f;
	{
		...
		window->DrawList->AddLine(ImVec2(separator_min_x, y), ImVec2(separator_max_x, y),
					  col, 1.0f);
	}
	return true;
}
...
static void ImGui::TabBarLayout(ImGuiTabBar *tab_bar)
{
	...
	if (width_excess > 0.0f && ((tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyResizeDown) ||
		...
		int shrink_data_offset =
			(central_section_is_visible ? sections[0].TabCount + sections[2].TabCount :
						      0);
					   ...
					   ImVec2 mouse_pos)
{
	...
	for (int i = src_idx; i >= 0 && i < tab_bar->Tabs.Size; i += dir) {
		...
		const float x2 =
			bar_offset + dst_tab->Offset + dst_tab->Width + g.Style.ItemInnerSpacing.x;
...
static ImGuiTabItem *ImGui::TabBarTabListPopupButton(ImGuiTabBar *tab_bar)
{
	...
	PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));
	bool open =
		BeginCombo("##v", NULL, ImGuiComboFlags_NoPreview | ImGuiComboFlags_HeightLargest);
	PopStyleColor(2);
		      ...
		      ImGuiTabItemFlags flags)
{
	...
	TabItemLabelAndCloseButton(display_draw_list, bb, flags, tab_bar->FramePadding, label, id,
				   close_button_id, tab_contents_visible, &just_closed,
				   &text_clipped);
